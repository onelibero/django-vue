<template>
	<div class="table-div">
		...
		>
            ...
            <el-table-column property="description" label="描述" align="center" sortable="custom" width="200" height="50">
                <!-- 设置表述文本框的大小 -->
            </el-table-column>
...
import ComicEdit from "@/components/ComicEdit.vue";
...
export default {
	...
	methods: {
		...
		initTableData: function () {
			var self = this;
			this.$Request
				.fetch_("/findcomic", "post", self.search) //后端url 分页查询
				.then((result) => {
                    console.log(result)
		...
		deleteModel: function (row) {
			...
			this.$confirm("确定要删除当前行吗？", "删除", {
				...
				.then(() => {
					console.log(row.id)
					var url = "/deletecomic?id=" + row.id;  //删除
					this.$Request
						.fetch_(url, "delete", {})
						.then((result) => {
							if (result == 200) {
